...
class Substitute(object):
    ...
    def __init__(self, item_similarity_model, ss_probability=0.05):
        self.item_similarity_model = item_similarity_model
        self.sse_threshold = 1.0 - ss_probability
        #self.sse_nums = substitute_nums

    def __call__(self, sequence):
        substitute_idx = range(len(sequence))
        #substitute_idx = random.sample([i for i in range(len(sequence))], k=self.sse_nums)
        for index in substitute_idx:
            if random.random() > self.sse_threshold:
                sequence[index] = self.item_similarity_model.most_similar(sequence[index])[0]
        return sequence
